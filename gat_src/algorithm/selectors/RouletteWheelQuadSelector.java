...
public class RouletteWheelQuadSelector extends SelectorStandard implements Selector {

	private FitnessRater fitnessRater;
	
	public RouletteWheelQuadSelector (int size, FitnessRater fitnessRater) {
		super(size);
		
		label = "Roulette Wheel Quad";
		description = "An implementation of Roulette Wheel selection, where the probabilities are squared.";
	...
	public ArrayList<Individual> selectChildren(Generation population) throws Exception {
		...
		for (Individual individual : population) {
			...
			wheel.put((double)Math.pow(fitness.value,2.0), individual);
		}
