...
public class TihaiComposerBeginningFocussed extends TihaiComposer implements Composer {
	...
	public TihaiComposerBeginningFocussed(BolBase bolBase, Tal tal, Variation variation) throws Exception {
		this(bolBase,tal,variation, null);
	}
	
	public TihaiComposerBeginningFocussed(BolBase bolBase, Tal tal, Variation variation, ThreadGroup threadGroup) throws Exception {
		super(bolBase,tal,variation,threadGroup);
	...
	private ArrayList<TihaiStructure> getTihaiStructures(Variation variation) {
		...
		while ((trunc*3) <= (nrOfStrokesInCycle-2)) {
			...
			if (Math.round(tailduration) == tailduration) {
				...
				if ((tailduration <= trunc) && ((tail*oneStroke)<4)){
						...
						for (int i=0; i < subSeqs.size(); i++) {
							...
							if (subSeqs.get(i).getDuration() == truncDuration) {
								...
								structs.add(new TihaiStructure(trunc,tail,style.speed, subSeqs.get(i)));
								...
								for (int j=i; j < subSeqs.size(); j++) {
									...
									if (dur == truncDuration) {
										...
										structs.add(new TihaiStructure(trunc,tail,style.speed, subSeqNew));
