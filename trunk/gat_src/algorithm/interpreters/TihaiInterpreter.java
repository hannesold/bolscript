...
/**
 * Renders a tihai to midi, uses an adapted Ge-Mapping and Subsubsequences.
 * @author Hannes
 *
 */
public class TihaiInterpreter extends VariationInterpreterStandard implements VariationInterpreter {
	 ...
	 */
	public TihaiInterpreter(BolBase base) {
		super();
		lateBolsInSubSeqStyle = new PlayingStyle(1, (double) 0.85); //0.35
		lateBolsInSeqStyle = new PlayingStyle(1,  (double) 0.9); //0.8
	...
	public BolSequence renderToBolSequenceAndMidiTrack(Variation variation, Track track, long startingAt) {
		...
		try {		
			...
			ArrayList subSeqs = variation.getSubSequencesRecursive(2);
			...
			for (int i=0; i < bolSeq.getLength(); i++) {
				...
				for (int j=0; j < midiMaps.length;j++) {
					...
					if ((mm.getHand() == MidiMap.NONE) || 
						...
						if (mm.getBolName().toString().equals("Ge")) {
							...
							if (subSeqPositions.indexOf(new Double(duration)) != -1) {
								//this time its a normal Ge
				...
				//out("\n");				
				//count on time
				duration = duration + (1.0/style.getSpeedValue());//Calc.roundIfClose(duration + (1.0/style.speed),4);
